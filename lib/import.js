  ...
  importMethod(methodName, file, pluginState) {
    if (!pluginState.selectedMethods[methodName]) {
      ...
      const path$1 = winPath(
        path.join(this.libraryName, libraryDirectory, transformedMethodName)
      );
      // 生成 import 语句
      // import Button from 'antd/lib/button'
      pluginState.selectedMethods[methodName] = helperModuleImports.addDefault(file.path, path$1, {
        nameHint: methodName,
      });
      if (style) {
        // 生成样式 import 语句
